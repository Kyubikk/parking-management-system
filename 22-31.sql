...
SELECT SUM(CONVERT(DECIMAL(10, 2), COALESCE(rv.total_fee, ph.total_fee))) AS total_revenue
FROM parkingHistory ph
LEFT JOIN registedVehicle rv ON ph.ticket_id = rv.ticket_id
WHERE MONTH(ph.entry_time) = MONTH(GETDATE())
AND YEAR(ph.entry_time) = YEAR(GETDATE());
...
AS
BEGIN
    INSERT INTO parkingHistory (vehicle_plate, vehicle_type, ticket_id, entry_time, exit_time, add_by, picture)
    SELECT vehicle_plate, vehicle_type, ticket_id, entry_time, GETDATE(), add_by, picture
    FROM deleted;
END;
...
SELECT SUM(CONVERT(DECIMAL(10, 2), COALESCE(rv.total_fee, ph.total_fee))) AS total_revenue FROM parkingHistory ph 
LEFT JOIN registedVehicle rv ON ph.ticket_id = rv.ticket_id 
WHERE MONTH(ph.entry_time) = MONTH(GETDATE())
AND YEAR(ph.entry_time) = YEAR(GETDATE());

SELECT COALESCE(SUM(CONVERT(DECIMAL(10, 2), COALESCE(rv.total_fee, ph.total_fee))), 0) AS total_revenue
FROM parkingHistory ph
LEFT JOIN registedVehicle rv ON ph.ticket_id = rv.ticket_id
WHERE MONTH(ph.entry_time) = MONTH(GETDATE())
  AND YEAR(ph.entry_time) = YEAR(GETDATE());
...
AS
BEGIN
    DECLARE @dailyFee FLOAT;
    DECLARE @registFee FLOAT;
    DECLARE @total FLOAT;

    SELECT @dailyFee = COALESCE(SUM(TRY_CAST(total_fee AS FLOAT)), 0)
    FROM parkingHistory ph
    WHERE MONTH(ph.exit_time) = MONTH(@month);

    SELECT @registFee = COALESCE(SUM(TRY_CAST(total_fee AS FLOAT)), 0)
    FROM registedVehicle rh
    WHERE MONTH(rh.start_at) = MONTH(@month);

    SET @total = @dailyFee + @registFee;

    RETURN @total;
END;

GO
SELECT dbo.calculateTotalRevenue(GETDATE()) AS TotalRevenue;
